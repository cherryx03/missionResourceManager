...
//                        }
    ...
    private JPanel createChartPanel(String title, String yAxis, double[] values, Color color) {
        XYSeriesCollection dataset = new XYSeriesCollection();
        XYSeries series = new XYSeries(title);;
        for (int j = 0; j < values.length; j++) {
            series.add(values[j], j);
        }
        dataset.addSeries(series);
        JFreeChart chart = ChartFactory.createXYLineChart(title, "Time [days]", yAxis, dataset);
